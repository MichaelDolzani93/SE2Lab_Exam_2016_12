...
/**
 * @brief check voto studenti
 * @return lista di stundenti con un voto minore o maggiore di quello indicato
 */
...
{	
	...
	//check body and parameters
	if ( typeof request.body !== 'undefined' && request.body){
		if ( typeof request.body.valore_inserito !== 'undefined' && request.body.valore_inserito){
			 valore_inserito = request.body.valore_inserito;
           	}else{
			valore_inserito = "not defined";
		}
	}else{
		valore_inserito = "body undefined";
	}
    
    	if (valore_inserito!="not defined" && valore_inserito!="body undefined"){
		//aceptable input
		var json;
		var symbol = valore_inserito[0];
		var mark = valore_inserito[1];
		var studenti = studentManager.searchMark(symbol, mark);
		//if insertion works correctly
		if(studenti.value == true){
                	json = JSON.stringify({ 
                    		studenti_lista: studenti.lista
                	});
            	}else{
                	json = JSON.stringify({ 
                    		studenti_lista: false
                	});
            	}
		response.writeHead(200, headers);
		response.end(json);
