...
var searchMark = function (symbol, marks){
	var res;
	var array = [];
	var num = 0;

	for (var i = 0; i < studentList.length; i++){
            	if (studentList[i].mark < marks && symbol == '<'){
			res = {
				value: true,
				studenti_lista: array[num] = studentList[i]
			}
			num++;
                }else if (studentList[i].mark > marks && symbol == '>'){
			res = {
				value: true,
				studenti_lista: array[num] = studentList[i]
			}
